...
def upgrade():
    # Ensure helpful index on existing certificate_acceptance
    op.execute("create index if not exists idx_ca_pon on certificate_acceptance (pon_id)")
    ...
    # Stock core (stores already exists from 0013)
    op.create_table(
        "skus",
        sa.Column("id", sa.dialects.postgresql.UUID(as_uuid=True), primary_key=True),
    ...
    op.execute(
        """
        insert into photo_tags (id, name, required_for_step)
        values
          (gen_random_uuid(), 'Dig', null),
          (gen_random_uuid(), 'Plant', null),
          (gen_random_uuid(), 'CA_Measurements', 'CAC'),
          (gen_random_uuid(), 'StringingStart', null),
          (gen_random_uuid(), 'Brackets', null),
          (gen_random_uuid(), 'Tension', null),
          (gen_random_uuid(), 'DeadEnds', null),
          (gen_random_uuid(), 'Completion', null)
