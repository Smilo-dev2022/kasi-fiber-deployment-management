...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("tech_id", sa.dialects.postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=True),
    ...
    op.create_index("idx_splices_tray_passed", "splices", ["tray_id", "passed"]) 
    ...
    op.create_index("idx_test_plans_link", "test_plans", ["link_name"]) 
    ...
    op.create_index("idx_otdr_plan", "otdr_results", ["test_plan_id"]) 
    op.create_index("idx_otdr_tested_at", "otdr_results", ["tested_at"]) 
    ...
    op.create_index("idx_lspm_plan", "lspm_results", ["test_plan_id"]) 
    op.create_index("idx_lspm_tested_at", "lspm_results", ["tested_at"]) 
    ...
    op.create_index("idx_connector_inspects_tested_at", "connector_inspects", ["tested_at"]) 
    ...
    op.create_table(
        ...
        sa.Column("type", sa.String(), nullable=True),  # 24C 48C etc
    ...
    op.create_index("idx_test_photos_kind", "test_photos", ["kind"]) 


def downgrade():
    op.drop_index("idx_test_photos_kind", table_name="test_photos")
    ...
    op.drop_index("idx_connector_inspects_tested_at", table_name="connector_inspects")
    ...
    op.drop_index("idx_lspm_tested_at", table_name="lspm_results")
    op.drop_index("idx_lspm_plan", table_name="lspm_results")
    op.drop_table("lspm_results")

    op.drop_index("idx_otdr_tested_at", table_name="otdr_results")
    op.drop_index("idx_otdr_plan", table_name="otdr_results")
    op.drop_table("otdr_results")

    op.drop_index("idx_test_plans_link", table_name="test_plans")
    ...
    op.drop_index("idx_splices_tray_passed", table_name="splices")
