...
def wards(db: Session = Depends(get_db)):
    ...
    feats = [{"type":"Feature","id":r["id"],"properties":{"name":r["name"]},"geometry":r["gj"] and __import__("json").loads(r["gj"])} for r in rows]
    return _fc(feats)
...
def pon_assets(pon_id: str, db: Session = Depends(get_db)):
    ...
    # Poles (optional table)
    try:
        rows = db.execute(text(
            """
      SELECT id, code, geom_geojson FROM poles WHERE pon_id=:p AND geom_geojson IS NOT NULL
    """
        ), {"p": pon_id}).mappings().all()
        feats += [{"type":"Feature","properties":{"type":"pole","id":str(r["id"]),"code":r["code"]},"geometry":r["geom_geojson"]} for r in rows]
    except Exception:
        pass
    # Closures
    rows = db.execute(text(
    ...
    feats += [{"type":"Feature","properties":{"type":"closure","id":str(r["id"]),"code":r["code"]},"geometry":r["geom_geojson"]} for r in rows]
    # Cables
    rows = db.execute(text(
        """
      SELECT id, cable_code, geom_geojson FROM cable_register WHERE pon_id=:p AND geom_geojson IS NOT NULL
    """
    ), {"p": pon_id}).mappings().all()
    feats += [{"type":"Feature","properties":{"type":"cable","id":str(r["id"]),"code":r["cable_code"]},"geometry":r["geom_geojson"]} for r in rows]
    return _fc(feats)
...
def incidents(
    ...
    where = ["i.geom_geojson IS NOT NULL", "i.status IN ('Open','Acknowledged')", "i.opened_at >= :since"]
    ...
    feats = [{"type":"Feature","properties":{"id":str(r["id"]),"severity":r["severity"],"category":r["category"],"title":r["title"]},"geometry": r["gj"] and __import__("json").loads(r["gj"])} for r in rows]
    return _fc(feats)
