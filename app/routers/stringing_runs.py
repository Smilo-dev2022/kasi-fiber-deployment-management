...
def create_run(payload: RunIn, db: Session = Depends(get_db)):
    ...
    db.execute(
        text(
            """
      insert into stringing_runs (id, pon_id, team_id, meters, brackets, dead_ends, tensioners, start_ts, end_ts, photos_ok, qc_passed, created_by)
      values (:id,:p,:t,:m,:b,:d,:te,:s,:e,:ph,:qc,:cb)
    """
        ),
        {
            "id": rid,
