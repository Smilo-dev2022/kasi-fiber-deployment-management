...
@router.get("/compliance-status/{smme_id}", dependencies=[Depends(require_roles("ADMIN", "PM"))])
def compliance_status(smme_id: str, db: Session = Depends(get_db)):
    ...
    def _is_valid(valid_to: Optional[str]) -> bool:
        if valid_to is None:
            return True
        try:
            return str(valid_to) >= str(_dt.date.today())
        except Exception:
            return False

    ok = all(r["verified"] and _is_valid(r["valid_to"]) for r in rows) if rows else False
