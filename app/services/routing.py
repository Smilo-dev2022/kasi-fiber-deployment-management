...
def find_assignment_org_id(db: Session, pon_id: Optional[UUID], ward: Optional[str], scope: str) -> Optional[UUID]:
    # Prefer PON assignment
    if pon_id:
        row = db.execute(
            text("select org_id from assignments where active = true and scope = :s and pon_id = :p order by priority asc limit 1"),
    ...
    if ward:
        row = db.execute(
            text("select org_id from assignments where active = true and scope = :s and ward = :w order by priority asc limit 1"),
            {"s": scope, "w": ward},
        ).first()
        if row:
            return row[0]
...
def contract_sla_minutes(db: Session, org_id: UUID, severity: str) -> int:
    ...
    return {"P1": row["sla_minutes_p1"], "P2": row["sla_minutes_p2"], "P3": row["sla_minutes_p3"], "P4": row["sla_minutes_p4"]}[severity]
